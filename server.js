...
app.post('/webhook/lead', async (req, res) => {
  ...
  try {
    ...
    if (existingConv.rows.length === 0) {
      ...
      await db.query(`
        UPDATE conversations SET product_interest = COALESCE($1, product_interest), last_message_at = NOW()
        WHERE id = $2
...
async function start() {
  await initDB();
  app.listen(PORT, () => {
    console.log(`🚀 Altech CRM corriendo en http://localhost:${PORT}`);
  });
}
