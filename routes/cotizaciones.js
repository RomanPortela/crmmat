...
// PATCH /api/cotizaciones/modelos/:id
router.patch('/modelos/:id', async (req, res) => {
  try {
    const { model_name, line, is_active } = req.body;
    const r = await db.query(`
      UPDATE cotizacion_models SET model_name = COALESCE($1,model_name),
        line = COALESCE($2,line), is_active = COALESCE($3,is_active)
      WHERE id=$4 RETURNING *
...
// DELETE /api/cotizaciones/modelos/:id
...
// PATCH /api/cotizaciones/entries/:id
router.patch('/entries/:id', async (req, res) => {
  try {
    const { storage_gb, battery_min, battery_max, base_price } = req.body;
    const r = await db.query(`
      UPDATE cotizacion_entries SET storage_gb = COALESCE($1,storage_gb),
        battery_min = COALESCE($2,battery_min), battery_max = COALESCE($3,battery_max),
        base_price = COALESCE($4,base_price)
      WHERE id=$5 RETURNING *
    `, [storage_gb, battery_min, battery_max, base_price, req.params.id]);
    res.json(r.rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
